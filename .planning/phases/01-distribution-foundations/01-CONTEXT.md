...
- **D-12:** Sparkle appcast XML is published to **`https://puffpuff.dev/molebar/appcast.xml`** via the repo's `gh-pages` branch (GitHub Pages auto-publish). CI commits the EdDSA-signed appcast.xml to `gh-pages` on every release. URL is locked into `Info.plist`'s `SUFeedURL` — changing it later strands existing users.
  - **Updated 2026-04-28** (during plan 01-01 execution, pre-first-release): originally `https://romatroskin.github.io/molebar/appcast.xml`. Plan 01-01's GitHub Pages enablement revealed that an account-level custom domain `puffpuff.dev` on `github.com/settings/pages` forces every `romatroskin.github.io/*` URL to 301 → `puffpuff.dev/*`. Sparkle 2.x follows redirects, so the original URL would still functionally resolve, but the FROZEN promise would silently depend on `puffpuff.dev` staying registered. Since v0.0.1 has not shipped, the URL is rotated to the actual serving domain so the FROZEN promise is self-contained. See `01-01-SUMMARY.md` Deviation 1.
...
- **A1 amendment — drop `SURequireSignedFeed`, `SUEnableInstallerLauncherService`, and `SUEnableDownloaderService` from Info.plist for Phase 1.** `SURequireSignedFeed` is for separate appcast-feed-level signing in older Sparkle paradigms; Sparkle 2.x EdDSA verification of release enclosures does NOT require it. The two `SUEnable*Service` keys are XPC service registrations needed only for **sandboxed** apps — MoleBar is unsandboxed (PROJECT.md: "App Store distribution out of scope; sandbox forbids deep-clean operations"), so those XPC services aren't relevant. Phase 1 Info.plist includes only `SUFeedURL`, `SUPublicEDKey`, `SUEnableAutomaticChecks`, plus standard bundle keys (`CFBundleIdentifier`, `LSUIElement=YES`, `MARKETING_VERSION`, `CURRENT_PROJECT_VERSION` placeholders).
